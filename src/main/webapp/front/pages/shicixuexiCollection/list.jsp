        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">诗词学习收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">诗词学习编号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="shicixuexiUuidNumber" id="shicixuexiUuidNumber" placeholder="诗词学习编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shicixuexiName" id="shicixuexiName" placeholder="成语名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                                                                 
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">诗词学习类型</label>
                              <div class="layui-input-inline">
                                  <select name="shicixuexiTypes" id="shicixuexiTypes" lay-filter="shicixuexiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in shicixuexiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteShicixuexiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../shicixuexi/detail.jsp?id='+item.shicixuexiId , item.shicixuexiDelete == null?'':item.shicixuexiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.shicixuexiPhoto?item.shicixuexiPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.shicixuexiName}}
                                        <span v-if="item.shicixuexiNewMoney"> - ￥{{item.shicixuexiNewMoney}}</span>
                                        <span v-else> - {{item.shicixuexiValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            shicixuexiCollectionTypesList : [],
        // 级联表字典表数据
            shicixuexiTypesList : [],
            shangxiaTypesList : [],
            shicixuexiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteShicixuexiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('shicixuexiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function shicixuexiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shicixuexi_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shicixuexiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function shicixuexiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shicixuexi_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shicixuexiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#shicixuexiUuidNumber').val() != null && jquery('#shicixuexiUuidNumber').val() != "" && jquery('#shicixuexiUuidNumber').val() != "null" ) {
                param['shicixuexiUuidNumber'] = jquery('#shicixuexiUuidNumber').val();
            }
                     
            if (jquery('#shicixuexiName').val() != null && jquery('#shicixuexiName').val() != "" && jquery('#shicixuexiName').val() != "null" ) {
                param['shicixuexiName'] = jquery('#shicixuexiName').val();
            }
                                                                                 
            var shicixuexiTypes = $("#shicixuexiTypes").find("option:selected").val();
            if (shicixuexiTypes != null && shicixuexiTypes != "" && shicixuexiTypes != "null") {
                            param['shicixuexiTypes'] = shicixuexiTypes;
                        }
                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['shicixuexiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('shicixuexiCollection/page', 'get', param, function(res) {
