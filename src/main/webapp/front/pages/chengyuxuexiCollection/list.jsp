        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">成语学习收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">成语学习编号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="chengyuxuexiUuidNumber" id="chengyuxuexiUuidNumber" placeholder="成语学习编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">成语名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="chengyuxuexiName" id="chengyuxuexiName" placeholder="成语名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                                                                 
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">成语学习类型</label>
                              <div class="layui-input-inline">
                                  <select name="chengyuxuexiTypes" id="chengyuxuexiTypes" lay-filter="chengyuxuexiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in chengyuxuexiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteChengyuxuexiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../chengyuxuexi/detail.jsp?id='+item.chengyuxuexiId , item.chengyuxuexiDelete == null?'':item.chengyuxuexiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.chengyuxuexiPhoto?item.chengyuxuexiPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.chengyuxuexiName}}
                                        <span v-if="item.chengyuxuexiNewMoney"> - ￥{{item.chengyuxuexiNewMoney}}</span>
                                        <span v-else> - {{item.chengyuxuexiValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            chengyuxuexiCollectionTypesList : [],
        // 级联表字典表数据
            chengyuxuexiTypesList : [],
            shangxiaTypesList : [],
            chengyuxuexiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteChengyuxuexiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('chengyuxuexiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function chengyuxuexiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chengyuxuexi_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.chengyuxuexiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function chengyuxuexiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chengyuxuexi_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.chengyuxuexiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#chengyuxuexiUuidNumber').val() != null && jquery('#chengyuxuexiUuidNumber').val() != "" && jquery('#chengyuxuexiUuidNumber').val() != "null" ) {
                param['chengyuxuexiUuidNumber'] = jquery('#chengyuxuexiUuidNumber').val();
            }
                     
            if (jquery('#chengyuxuexiName').val() != null && jquery('#chengyuxuexiName').val() != "" && jquery('#chengyuxuexiName').val() != "null" ) {
                param['chengyuxuexiName'] = jquery('#chengyuxuexiName').val();
            }
                                                                                 
            var chengyuxuexiTypes = $("#chengyuxuexiTypes").find("option:selected").val();
            if (chengyuxuexiTypes != null && chengyuxuexiTypes != "" && chengyuxuexiTypes != "null") {
                            param['chengyuxuexiTypes'] = chengyuxuexiTypes;
                        }
                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['chengyuxuexiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('chengyuxuexiCollection/page', 'get', param, function(res) {
