        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">情景学习收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">情景学习编号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="qingjingxuexiUuidNumber" id="qingjingxuexiUuidNumber" placeholder="情景学习编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">情景学习名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="qingjingxuexiName" id="qingjingxuexiName" placeholder="情景学习名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                                                                 
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">情景学习类型</label>
                              <div class="layui-input-inline">
                                  <select name="qingjingxuexiTypes" id="qingjingxuexiTypes" lay-filter="qingjingxuexiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in qingjingxuexiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteQingjingxuexiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../qingjingxuexi/detail.jsp?id='+item.qingjingxuexiId , item.qingjingxuexiDelete == null?'':item.qingjingxuexiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.qingjingxuexiPhoto?item.qingjingxuexiPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.qingjingxuexiName}}
                                        <span v-if="item.qingjingxuexiNewMoney"> - ￥{{item.qingjingxuexiNewMoney}}</span>
                                        <span v-else> - {{item.qingjingxuexiValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            qingjingxuexiCollectionTypesList : [],
        // 级联表字典表数据
            qingjingxuexiTypesList : [],
            shangxiaTypesList : [],
            qingjingxuexiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteQingjingxuexiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('qingjingxuexiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function qingjingxuexiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=qingjingxuexi_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.qingjingxuexiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function qingjingxuexiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=qingjingxuexi_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.qingjingxuexiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#qingjingxuexiUuidNumber').val() != null && jquery('#qingjingxuexiUuidNumber').val() != "" && jquery('#qingjingxuexiUuidNumber').val() != "null" ) {
                param['qingjingxuexiUuidNumber'] = jquery('#qingjingxuexiUuidNumber').val();
            }
                     
            if (jquery('#qingjingxuexiName').val() != null && jquery('#qingjingxuexiName').val() != "" && jquery('#qingjingxuexiName').val() != "null" ) {
                param['qingjingxuexiName'] = jquery('#qingjingxuexiName').val();
            }
                                                                                 
            var qingjingxuexiTypes = $("#qingjingxuexiTypes").find("option:selected").val();
            if (qingjingxuexiTypes != null && qingjingxuexiTypes != "" && qingjingxuexiTypes != "null") {
                            param['qingjingxuexiTypes'] = qingjingxuexiTypes;
                        }
                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['qingjingxuexiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('qingjingxuexiCollection/page', 'get', param, function(res) {
