        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">基础收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">基础编号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="jichuUuidNumber" id="jichuUuidNumber" placeholder="基础编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">基础名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="jichuName" id="jichuName" placeholder="基础名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="jichuTypes" id="jichuTypes" lay-filter="jichuTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in jichuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteJichuCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../jichu/detail.jsp?id='+item.jichuId , item.jichuDelete == null?'':item.jichuDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.jichuPhoto?item.jichuPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.jichuName}}
                                        <span v-if="item.jichuNewMoney"> - ￥{{item.jichuNewMoney}}</span>
                                        <span v-else> - {{item.jichuValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            jichuCollectionTypesList : [],
        // 级联表字典表数据
            jichuTypesList : [],
            shangxiaTypesList : [],
            jichuCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteJichuCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jichuCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function jichuCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jichu_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jichuCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function jichuTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jichu_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jichuTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#jichuUuidNumber').val() != null && jquery('#jichuUuidNumber').val() != "" && jquery('#jichuUuidNumber').val() != "null" ) {
                param['jichuUuidNumber'] = jquery('#jichuUuidNumber').val();
            }
                     
            if (jquery('#jichuName').val() != null && jquery('#jichuName').val() != "" && jquery('#jichuName').val() != "null" ) {
                param['jichuName'] = jquery('#jichuName').val();
            }
                                                                                 
            var jichuTypes = $("#jichuTypes").find("option:selected").val();
            if (jichuTypes != null && jichuTypes != "" && jichuTypes != "null") {
                            param['jichuTypes'] = jichuTypes;
                        }
                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['jichuCollectionTypes'] = 1;

            // 获取列表数据
            http.request('jichuCollection/page', 'get', param, function(res) {
